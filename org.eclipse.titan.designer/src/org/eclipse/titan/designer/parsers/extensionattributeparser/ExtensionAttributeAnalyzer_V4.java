...
public class ExtensionAttributeAnalyzer_V4 extends ExtensionAttributeAnalyzer {
	...
	public void parse(AttributeSpecification specification) {
		...
		ExtensionAttributeParser2 parser = new ExtensionAttributeParser2(tokens);
		...
		if (!lexerListener.getErrorsStored().isEmpty()) {
			...
			for (int i = 0; i < lexerListener.getErrorsStored().size(); i++) {
				...
				ParserMarkerSupport_V4.createOnTheFlyMixedMarker((IFile) location.getFile(), lexerListener.getErrorsStored().get(i), errorLevel, temp);
			}
		}
		if (!parserListener.getErrorsStored().isEmpty()) {
			...
			for (int i = 0; i < parserListener.getErrorsStored().size(); i++) {
				Location temp = new Location(location);
				temp.setOffset(temp.getOffset() + 1);
				ParserMarkerSupport_V4.createOnTheFlyMixedMarker((IFile) location.getFile(), parserListener.getErrorsStored().get(i), errorLevel, temp);
			}
		}
	}
