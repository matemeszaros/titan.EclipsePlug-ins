...
@header {
	...
	import org.eclipse.titan.designer.AST.ASN1.types.ASN1_Enumerated_Type_V4;
	import org.eclipse.titan.designer.AST.ASN1.types.ASN1_Set_Type_V4;
	import org.eclipse.titan.designer.AST.ASN1.types.ASN1_BitString_Type_V4;
	import org.eclipse.titan.designer.AST.ASN1.types.ASN1_Choice_Type_V4;
	import org.eclipse.titan.designer.AST.ASN1.types.ASN1_Sequence_Type_V4;
	import org.eclipse.titan.designer.AST.ASN1.types.ASN1_Integer_Type_V4;
	import org.eclipse.titan.designer.AST.ASN1.Parameterised_Reference_V4;
	import org.eclipse.titan.designer.AST.ASN1.InformationFromObj_V4;
	import org.eclipse.titan.designer.AST.ASN1.TableConstraint_V4;
	import org.eclipse.titan.designer.AST.ASN1.Undefined_Assignment_OS_or_VS_V4;
	import org.eclipse.titan.designer.AST.ASN1.ValueSet_Assignment_V4;
}
...
@members{
	...
	private Location getLocation(final Token token) {
		return new Location_V4(actualFile, line + token.getLine() - 1, offset + token.getStartIndex(), offset + token.getStopIndex() + 1);
	}
	...
	private Location getLocation(final Token startToken, final Token endToken) {
		...
		return new Location_V4(actualFile, line - 1 + startToken.getLine(), offset + startToken.getStartIndex(), offset + endToken.getStopIndex() + 1);
	}
	...
	public Token consume() {
			...
			if(token.getType() == BLOCK) {
				consume_counter += ((TokenWithIndexAndSubTokensV4)token).tokenList.size() + 2;
			} else {
				consume_counter++;
			}
...
(	
	(	
		a = pr_Type_reg
		{
			...
			$assignment.setLocation(new Location_V4(actualFile, $a.start, $a.start));
		}
	|	b = pr_FromObjs
		{
			$reference = $b.fromObj; 
			...
			$type.setLocation(new Location_V4(actualFile, $b.start, $b.start));
			$assignment = new Type_Assignment($identifier, null, $type);
			$assignment.setLocation(new Location_V4(actualFile, $col, $col));
		}
	|	c = pr_UpperFromObj
		{
			$reference = $c.fromObj;
			$col = $c.start;
			$type = new Referenced_Type($reference);
			$type.setLocation(new Location_V4(actualFile, $c.start, $c.start));
			$assignment = new Type_Assignment($identifier, null, $type);
			$assignment.setLocation(new Location_V4(actualFile, $col, $col));
		}
	|	d = pr_ObjectClassDefn
		{
			...
			$assignment.setLocation(new Location_V4(actualFile, $d.start, $d.start));
		}
	)
	EOF
);
...
@init { $identifier = null; $defaultTagging = null; $assignments = null; }:
	...
	EOF
{
	act_asn1_module.setLocation(new LargeLocation_V4(actualFile, $start, $f));
	if($assignments != null) {
		$assignments.setLocation(new Location_V4(actualFile, $d, $f));
...
@init { $identifier = null; }:
	a = UPPERIDENTIFIER
{
	$identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
};
...
(
	...
	|	{
			$imports = new Imports();
			Location location = new Location_V4(actualFile);
...
{
	if ($assignment != null) {
		$assignment.setLocation(new Location_V4(actualFile, $col, $endcol));
	}
};
...
)
{
	...
	new Location_V4(actualFile, $b, $b)), $symbols);
};
...
(
	id = UPPERIDENTIFIER	{ $identifier = new Identifier(Identifier_type.ID_ASN, $id.getText(), new Location_V4(actualFile, $id, $id)); }
...
|	(
		uo = pr_UpperFromObj  
		{
			...
			$type = new Referenced_Type($reference);
			$type.setLocation(new Location_V4(actualFile, $uo.start, $uo.start));
			$assignment = new Type_Assignment($identifier, $formalParameters, $type);
...
|	(
		fo = pr_FromObjs  
		{
			...
			$type = new Referenced_Type($reference);
			$type.setLocation(new Location_V4(actualFile, $fo.start, $fo.start));
			$assignment = new Type_Assignment($identifier, $formalParameters, $type);
		}
	)
);
...
/*{
	if ($objectClass_Definition != null) {
		$assignment.setLocation(new Location_V4(actualFile, $id, $endCol));
	}
}*/
;
...
(
	...
	(
		(
			...
			b1 = BLOCK			
			{
				$assignment = new Undefined_Assignment_OS_or_VS_V4($identifier, $formalParameters, $reference, new BlockV4($b1));
			}
		)
	|	(
			ASSIGNMENT
...
|	(
		id = LOWERIDENTIFIER	{ $identifier = new Identifier(Identifier_type.ID_ASN, $id.getText(), new Location_V4(actualFile, $id, $id)); }
		fp = pr_OptParList		{ $formalParameters = $fp.parameters; }
		...
		(
			(
				...
				{
					if($reference != null) {
						$assignment = new Undefined_Assignment_O_or_V($identifier, $formalParameters, $reference, new BlockV4($b3));
					}
				}
			)
		|	(
...
(
	...
	b = BLOCK
	{
		$assignment = new ValueSet_Assignment_V4($identifier, $formalParameters, $type, new BlockV4($b));
		reportWarning(new TITANMarker_V4("Taking ValueSetTypeAssignment as a TypeAssignment",
					$id, $id, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
	}
);
...
(
	...
	(
		(
			...
			|	block1 = BLOCK				{ $value = new Undefined_Block_Value_V4(new BlockV4($block1)); $endcol = $block1;}
...
(
	(
		b = BLOCK	{ if($b != null) { $parameters = new Ass_pard_V4(new BlockV4($b)); }}
	)?
);
	...
	(	(	
		...
		)   // pr_SeqOfSetOfWithConstraint, pr_SequenceOfType, pr_SetOfType
		{
			if($set != null) {
				...
				$type.setLocation(new Location_V4(actualFile, $set, $c.start != null ? $c.start : $d.start));
			} else if($sequence != null) {
				$type = new SequenceOf_Type($subType);
				$type.setLocation(new Location_V4(actualFile, $sequence, $c.start != null ? $c.start : $d.start));
			}
...
//		(
			(	
				...
				{
					$type.setLocation(new Location_V4(actualFile, $builtin.start, $builtin.start));
					$type.addConstraints($constraints);
				}
			|	ref = pr_ReferencedType { $type = $ref.type; }
				f = pr_Constraints
				{
					...
					$type.setLocation(new Location_V4(actualFile, $ref.start, $ref.start));
					$type.addConstraints($constraints);
				}
			)
/*		(	(	pr_ReferencedType
...
locals [Token end, BlockV4 withBlock]
...
{
	$objectClass_Definition = new ObjectClass_Definition_V4(new BlockV4($block1), $withBlock);
	$objectClass_Definition.setLocation(new Location_V4(actualFile, $block1, $end));
};
...
(	
	...
	|	b = LOWERIDENTIFIER
		{ 
			$identifier = new Identifier(Identifier_type.ID_ASN, $b.getText(), new Location_V4(actualFile, $b, $b));
		}
	)
	( 
		BLOCK
...
pr_WithSyntaxSpec returns [BlockV4 block]
@init { $block = null; }:
(	
	WITH
	SYNTAX
...
(
	startCol = pr_FieldSpec { $fieldSpecification = $startCol.fieldSpecification; }
	{
		if ($fieldSpecification != null) {
			$fieldSpecification.setLocation(new Location_V4(actualFile, $startCol.start, $startCol.start));
	...
	(	
		...
		endCol = pr_FieldSpec { $fieldSpecification = $endCol.fieldSpecification; }
			{	if($fieldSpecification != null) {
					$fieldSpecification.setLocation(new Location_V4(actualFile, $endCol.start, $endCol.start));
					$fieldSpecifications.addFieldSpecification($fieldSpecification);
				}
			}
	)*
...
{
	Identifier identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
...
locals [Identifier identifier, Defined_Reference reference, boolean is_optional, BlockV4 defaultBlock, Reference defaultSetting]
@init { $identifier = null; $reference = null;  $is_optional = false; $defaultBlock = null; $defaultSetting = null; 
		$fieldSpecification = null; } :
(
	(	
		a = AMPLOWERIDENTIFIER	{ $identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a)); }
	|	b = AMPUPPERIDENTIFIER	{ $identifier = new Identifier(Identifier_type.ID_ASN, $b.getText(), new Location_V4(actualFile, $b, $b)); }
	)
	...
	|	DEFAULT
		(	
			block = BLOCK 	{ $defaultBlock = new BlockV4($block); }//pr_Dflt_Block
...
(	
	(
		block = BLOCK
		{
			ObjectClassSyntax_Builder builder = new ObjectClassSyntax_Builder_V4(new BlockV4($block), $fieldSpecifications);
	...
	|	(	
			c = AMPUPPERIDENTIFIER { $identifier = new Identifier(Identifier_type.ID_ASN, $c.getText(), new Location_V4(actualFile, $c, $c)); $endcol = $c; }
		|	d = AMPLOWERIDENTIFIER { $identifier = new Identifier(Identifier_type.ID_ASN, $d.getText(), new Location_V4(actualFile, $d, $d)); $endcol = $d; }
		)	//ampId
		{
			if ($identifier != null) {
				if ($fieldSpecifications.hasFieldSpecificationWithId($identifier)) {
					...
					(new Location_V4(actualFile, $endcol, $endcol)).reportSemanticError("No field with name `" + $identifier.getDisplayName() + "'");
				}
			}
		}
	)*
...
(	
	(
		(	a1 = LOWERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $a1.getText(), new Location_V4(actualFile, $a1, $a1)));
				$subReference.setLocation(getLocation($a1));
			}
		|	b1 = UPPERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $b1.getText(), new Location_V4(actualFile, $b1, $b1)));
				$subReference.setLocation(getLocation($b1));
			}
		|	c1 = AMPLOWERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $c1.getText(), new Location_V4(actualFile, $c1, $c1)));
				$subReference.setLocation(getLocation($c1));
			}
		|	d1 = AMPUPPERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $d1.getText(), new Location_V4(actualFile, $d1, $d1)));
	...
	(	
		...
		(	a2 = LOWERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $a2.getText(), new Location_V4(actualFile, $a2, $a2)));
				$subReference.setLocation(getLocation($a2));
			}
		|	b2 = UPPERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $b2.getText(), new Location_V4(actualFile, $b2, $b2)));
				$subReference.setLocation(getLocation($b2));
			}
		|	c2 = AMPLOWERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $c2.getText(), new Location_V4(actualFile, $c2, $c2)));
				$subReference.setLocation(getLocation($c2));
			}
		|	d2 = AMPUPPERIDENTIFIER
			{
				$subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $d2.getText(), new Location_V4(actualFile, $d2, $d2)));
...
)
{
	...
	$enumItem.setLocation(new Location_V4(actualFile, $a, $d == null ? $a : $d));
};
...
)
{
	...
	$namedValue.setLocation(new Location_V4(actualFile, $a, $d == null ? $a : $d));
};
...
(
	a = LOWERIDENTIFIER
	{
		Identifier identifier1 = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	...
	(	
		...
		b = LOWERIDENTIFIER
		{
			Identifier identifier2 = new Identifier(Identifier_type.ID_ASN, $b.getText(), new Location_V4(actualFile, $b, $b));
			$named_bits.addId(identifier2);
		}
	)*
)
{
	$named_bits.setLocation(new Location_V4(actualFile, $a, $b == null ? $a : $b));
};
...
)
{
	Identifier identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	$namedValue = new NamedValue(identifier, value);
	$namedValue.setLocation(new Location_V4(actualFile, $a, $endcol == null ? $a : $endcol));
};
...
pr_DefdLower
{
	...
	$value.setLocation(new Location_V4(actualFile, $start, $pr_DefdLower.stop));
};
...
a = NUMBER
{
	...
	$value.setLocation(new Location_V4(actualFile, $start, $a));
};
...
)
{
	if($b == null) {
		...
		$reference =  new Parameterised_Reference_V4(simpleReference, new BlockV4($b));
	}
	if($reference != null) {
		$reference.setLocation(new Location_V4(actualFile, $start, $b == null ? $start: $b));
	}
};
...
{
	id1 = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	$reference = new Defined_Reference(id1);
	FieldSubReference subReference = new FieldSubReference(new Identifier(Identifier_type.ID_ASN, $b.getText(), new Location_V4(actualFile, $b, $b)));
	subReference.setLocation(new Location_V4(actualFile, $b, $b));
	$reference.addSubReference(subReference);
};
...
{
	$id = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	FieldSubReference subReference = new FieldSubReference($id);
	subReference.setLocation(new Location_V4(actualFile, $a, $a));
...
(	
	(	
		...
		)
		{
			if ($set != null) {
				$type = new SetOf_Type($subType);
				$type.setLocation(new Location_V4(actualFile, $set, $namedTypecol.start != null ? $namedTypecol.start : $subTypecol.start));
			} else if ($sequence != null) {
				$type = new SequenceOf_Type($subType);
				$type.setLocation(new Location_V4(actualFile, $sequence, $namedTypecol.start != null ? $namedTypecol.start : $subTypecol.start));
			}
			if ($constraint != null && $type != null) {
				$constraints = new Constraints();
				$constraints.addConstraint($constraint);
...
|	(
		...
		|	referenced = pr_ReferencedType
			{	
				...
				$type.setLocation(new Location_V4(actualFile, $referenced.start, $referenced.start));
			}
		)//pr_NakedType_reg
		(
			a = pr_Constraints
...
{
	$helper.identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	$helper.type = type;
};
...
{
	$objectSet = new ObjectSet_definition_V4( new BlockV4($block) );
	$objectSet.setLocation(new Location_V4(actualFile, $block, $block));
};
...
|	block = BLOCK { $value = new Undefined_Block_Value_V4(new BlockV4($block)); }
);
...
)
{
	...
	$value.setLocation(new Location_V4(actualFile, $a.start, $a.start));
};
...
{
	$type = new Referenced_Type($reference);
	$type.setLocation(new Location_V4(actualFile, $a.start, $a.start));
};
...
{
	$object = new Object_Definition( new BlockV4($block) );
	$object.setLocation(new Location_V4(actualFile, $block, $block));
};
...
(	
	a = BSTRING
	{
		...
		$value.setLocation(new Location_V4(actualFile, $a, $a));
	}
|	b = HSTRING
	{
		$value = new Hexstring_Value($b.getText());
		$value.setLocation(new Location_V4(actualFile, $b, $b));
	}
		...
		|	f11 = REALNUMBER	
			{
				...
				$value.setLocation(new Location_V4(actualFile, $f11, $f11));
			}
		)
		...
		|	f13 = REALNUMBER
			{
				...
				$value.setLocation(new Location_V4(actualFile, $f13, $f13));
			}
		)
	)
|	g = PLUS_INFINITY
	{
		...
		$value.setLocation(new Location_V4(actualFile, $g, $g));
	}
|	h = MINUS_INFINITY
	{
		$value = new Real_Value(Float.NEGATIVE_INFINITY);
		$value.setLocation(new Location_V4(actualFile, $h, $h));
	}
|   i = NOT_A_NUMBER
	{
		$value = new Real_Value(Float.NaN);
		$value.setLocation(new Location_V4(actualFile, $i, $i));
	}
);
...
@init { $value = null; }:
	a = LOWERIDENTIFIER
{
	Location location = new Location_V4(actualFile, $a, $a);
...
{
	$type = new NULL_Type();
	$type.setLocation(new Location_V4(actualFile, $start, $start));
};
...
{
	$type = new ASN1_BitString_Type_V4($c != null ? new BlockV4($c):null);
	$type.setLocation(new Location_V4(actualFile, $a, $endCol));
};
...
{
	$type = new ASN1_Choice_Type_V4($endCol != null ? new BlockV4($endCol) : null);
	$type.setLocation(new Location_V4(actualFile, $start, $endCol));
};
...
{
	Identifier identifier = new Identifier(Identifier_type.ID_ASN, $start.getText(), new Location_V4(actualFile, $start, $start));
	$type = new Selection_Type(identifier, $a.type);
	$type.setLocation(new Location_V4(actualFile, $start, $a.start));
};
...
{
	$type = new ASN1_Enumerated_Type_V4($block != null ? new BlockV4($block) : null);
	$type.setLocation(new Location_V4(actualFile,  $start, $block));
};
...
{
	$type = new ASN1_Integer_Type_V4($b != null ? new BlockV4($b) : null);
	$type.setLocation(new Location_V4(actualFile, $start, $b == null ? $start:$b));
};
...
)
{
	...
	$type.setLocation(new Location_V4(actualFile, $start, $a));
};
...
{
	$type = new ASN1_Sequence_Type_V4($block != null ? new BlockV4($block):null);
...
{
	$type = new ASN1_Set_Type_V4($block != null ? new BlockV4($block):null);
	$type.setLocation(new Location_V4(actualFile, $start, $block));
};
...
{
	if ($type != null) {
		$type.setLocation(new Location_V4(actualFile, $col, $col));
	}
};
...
{
	$constraint = new TableConstraint_V4(new BlockV4($objectSetBlock), new BlockV4($atNotationsBlock));
};
...
(
	...
	(
		...
		b = pr_FieldNameLower //	pr_LowerFromObj
			{
				$reference = new InformationFromObj_V4($a.reference, $b.fieldName);
			}
...
{
	if ($value != null) {
		$value.setLocation(new Location_V4(actualFile, $col, $col));
	}
};

pr_ChoiceValue returns [Choice_Value value]
@init {Identifier identifier = null; Value alternative = null;  $value = null; }:
(
	LOWERIDENTIFIER
	COLON
	a = pr_Value
...
{
	identifier = new Identifier(Identifier_type.ID_ASN, $start.getText(), new Location_V4(actualFile, $start, $start));
	if (alternative != null) {
		$value = new Choice_Value(identifier, $a.value);
		$value.setLocation(new Location_V4(actualFile, $start, $a.start));
	}
};
...
(
	a = AMPLOWERIDENTIFIER
		{
			Location location1 = new Location_V4(actualFile, $a, $a);
	...
	(
		...
		{
			Location location2 = new Location_V4(actualFile, $b, $b);
...
{
	$fromObj = new InformationFromObj_V4($a.reference, $b.fieldName);
	$fromObj.setLocation(new Location_V4(actualFile, $a.start, $b.start));
};
...
(
	...
	(	
		block = BLOCK
		{
			if($a.reference != null) { $reference = new Parameterised_Reference_V4($a.reference, $block != null ? new BlockV4($block):null); }
		}
	|	{ $reference = $a.reference; }
	)
)
{
	if($reference != null){
		$reference.setLocation(new Location_V4(actualFile, $a.start, $block != null ? $block:$a.start));
	}
};
...
(	
	a = UPPERIDENTIFIER
	{	
		first = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	}
	(
		DOT
		b = UPPERIDENTIFIER
...
{
	if ($reference != null) {
		$reference.setLocation(new Location_V4(actualFile, $a, $b != null ? $b:$a));
	}
};
...
(
	...
	(
		...
		{
			$ref = new Parameterised_Reference_V4($a.reference, new BlockV4($a.start));
			$end = $block;
		}
	)?
	DOT
	...
	|	b2 = pr_FieldNameUpper	{ $end = $b2.start; $fieldName = $b2.fieldName; }
		(	
			...
			(	
				...
				{
					Location location = new Location_V4(actualFile, $c, $c);
			...
			|	d = AMPUPPERIDENTIFIER
				{
					Location location = new Location_V4(actualFile, $d, $d);
...
{
	$fromObj = new InformationFromObj_V4($ref, $fieldName);
	$fromObj.setLocation(new Location_V4(actualFile, $a.start, $end));
};
...
(	
	...
	(	
		...
		|	b = AMPUPPERIDENTIFIER
			{
				$endcol = $b;
				Location location = new Location_V4(actualFile, $b, $b);
...
|	block = BLOCK	{ $constraint = new TableConstraint_V4(new BlockV4($block), null); $col = $block;}
//|	pr_ReferencedValue_reg
);
...
(
	id = AMPLOWERIDENTIFIER	{ $identifier = new Identifier(Identifier_type.ID_ASN, $id.getText(), new Location_V4(actualFile, $id, $id));}
	(
		( 
	...
	|	(
			...
			(
				block = BLOCK	{ $reference = new Parameterised_Reference_V4($simpleReference, new BlockV4($block)); $referenceEnd = $block; }//pr_PardUpper
				(
					(
						...
						|	DEFAULT			{ $has_default = true; }
							...
							|	defaultBlock1 = BLOCK { $defaultValue = new Undefined_Block_Value_V4(new BlockV4($defaultBlock1));
														$defaultValue.setLocation(new Location_V4(actualFile, $defaultBlock1, $defaultBlock1)); }//pr_Dflt_Block
							...
							|	lowerid1 = LOWERIDENTIFIER 
								{
									Identifier tempId = new Identifier(	Identifier_type.ID_ASN, $lowerid1.getText(),
																		new Location_V4(actualFile, $lowerid1, $lowerid1));
																		$defaultValue = new Undefined_LowerIdentifier_Value(tempId);
																		$defaultValue.setLocation(new Location_V4(actualFile, $lowerid1, $lowerid1));
								}//pr_Dflt_LowerId
							)
						)?
					)
				|	(
					...
					|	d2 = DEFAULT	{ $has_default = true; }
						...
						|	defaultBlock2 = BLOCK {	$defaultValue = new Undefined_Block_Value_V4(new BlockV4($defaultBlock2));
													$defaultValue.setLocation(new Location_V4(actualFile, $defaultBlock2, $defaultBlock2));}//pr_Dflt_Block
						)
					|	pr_RefdLower_reg
					|	DOT
						(
						...
						|	d42 = pr_FieldNameUpper	{ $fieldName = $d42.fieldName; }
							(
								...
								(
									...
									{
										Location location = new Location_V4(actualFile, $d411, $d411);
								...
								|	d412 = AMPUPPERIDENTIFIER
									{
										Location location = new Location_V4(actualFile, $d412, $d412);
						...
						{
							$reference = new InformationFromObj_V4((Defined_Reference)$reference, $fieldName);
							$reference.setLocation(new Location_V4(actualFile, $referenceStart.start, $referenceEnd));
						}
			...
			|	UNIQUE	{ $is_unique = true; $reference = $simpleReference; }
				...
				|	DEFAULT		{ $has_default = true; }
					...
					|	defaultBlock3 = BLOCK { $defaultValue = new Undefined_Block_Value_V4(new BlockV4($defaultBlock3));
												$defaultValue.setLocation(new Location_V4(actualFile, $defaultBlock3, $defaultBlock3)); }//pr_Dflt_Block
					|	pr_RefdLower_reg //pr_Dflt_RefdLower_reg
					|	lowerid2 = LOWERIDENTIFIER { 	Identifier tempId = new Identifier(Identifier_type.ID_ASN, $lowerid2.getText(), 
														new Location_V4(actualFile, $lowerid2, $lowerid2));
														$defaultValue = new Undefined_LowerIdentifier_Value(tempId);
														$defaultValue.setLocation(new Location_V4(actualFile, $lowerid2, $lowerid2)); }//pr_Dflt_LowerId
					)
				)?
			|	(	
					DEFAULT	{ $has_default = true; }
					...
					|	a21 = pr_FieldNameUpper { $fieldName = $a21.fieldName; }
						(	DOT
							(	
								...
								{
									Location location = new Location_V4(actualFile, $b2, $b2);
							...
							|	c2 = AMPUPPERIDENTIFIER
								{
									Location location = new Location_V4(actualFile, $c2, $c2);
...
)
{
	Identifier identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
	$namedValue = new NamedValue(identifier, $value);
	$namedValue.setLocation(new Location_V4(actualFile, $a, $b.start == null ? $a : $b.start));
};
...
|	b = LOWERIDENTIFIER	
	{
		Identifier identifier = new Identifier(Identifier_type.ID_ASN, $b.getText(), 
								new Location_V4(actualFile, $b, $b));
								$objidComponent = new ObjectIdentifierComponent(identifier, null);
								$objidComponent.setLocation(new Location_V4(actualFile, $b, $b));
	}
|	c = pr_DefinedValue_reg
	{
		...
		if ($value != null) {
			...
			$objidComponent.setLocation(new Location_V4(actualFile, $c.start, $c.start));
		}
	}
);
...
|	b = pr_Val_Number			{ $value = $b.value; }
	{
		if ($value != null) {
			$objidComponent = new ObjectIdentifierComponent($value);
			$objidComponent.setLocation(new Location_V4(actualFile, $b.start, $b.start));
		}
	}
);
...
)
{
	...
	$objidComponent.setLocation(new Location_V4(actualFile, $a, $d));
};
...
(
	...
	block1 = BLOCK	{ $reference = new Parameterised_Reference_V4($simpleReference, new BlockV4($block1) ); }
|	(
		b = pr_ExtLowerRef	{ $simpleReference = $b.reference; }	//pr_SimplDefdLower_reg
		(
			block2 = BLOCK										//pr_PardLower -> pr_SimplDefdLower
			{ $reference = new Parameterised_Reference_V4($simpleReference, new BlockV4($block2) ); }
		|	{ $reference = $simpleReference; }
		)
	)
);
...
pr_special_FormalParameterList returns [ArrayList<FormalParameter_Helper_V4> parameters]
locals [FormalParameter_Helper_V4 helper]
@init { $helper = null; $parameters = new ArrayList<FormalParameter_Helper_V4>(); }:
(
...
pr_FormalParameter returns [FormalParameter_Helper_V4 formalParameter]
@init { $formalParameter = new FormalParameter_Helper_V4(); }:
(	
	(
	...
	(	a = UPPERIDENTIFIER
		{	
			$formalParameter.identifier = new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a));
			$formalParameter.formalParameterToken = $a;
		}
	|	b = LOWERIDENTIFIER
		{	
			$formalParameter.identifier = new Identifier(Identifier_type.ID_ASN, $b.getText(), new Location_V4(actualFile, $b, $b));
...
|	COMPONENTS
	...
	b = pr_Type
	{
		...
		if ($type != null) {
			...
			$componentType.setLocation(new Location_V4(actualFile, $b.start, $b.start));
		}
	}
);
...
{
	if ($definition != null) { $definition.setLocation(new Location_V4(actualFile, $a.start, $a.start)); }
};
...
|	e = ELLIPSIS
	...
	|	{ $definition = new ObjectSet_definition_V4(); $definition.setLocation(new Location_V4(actualFile, $e, $e));}
	)
);
...
(	
	...
	{
		$element = $a.element;
		$definition = new ObjectSet_definition_V4();
...
{
	if ($definition != null) {
		$definition.setLocation(new Location_V4(actualFile, $a.start, $a.start == null ? $b.start : $a.start));
	}
};
...
|	b = pr_RefdUpper
	{
		...
		temp.setLocation(new Location_V4(actualFile, $b.start, $b.start));
		$element = temp;
	}
);
...
(	
	a = LOWERIDENTIFIER
	{
		...
		$fieldName.addField(new Identifier(Identifier_type.ID_ASN, $a.getText(), new Location_V4(actualFile, $a, $a)));
	}
	(
		DOT 
		b = LOWERIDENTIFIER
		{
			if ($fieldName != null) {
				$fieldName.addField(new Identifier(Identifier_type.ID_ASN, $b.getText(), new Location_V4(actualFile, $b, $b)));
			}
		}
	)*
);
