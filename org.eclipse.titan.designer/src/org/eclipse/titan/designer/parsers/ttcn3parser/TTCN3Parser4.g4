...
public void setLexer(final TTCN3Lexer4 aLexer) {
...
public void setLexer(final TTCN3KeywordLessLexer4 aLexer) {
	this.lexer = new ILexer() {
		@Override
		public Location getLastCommentLocation() {
			return aLexer.getLastCommentLocation();
...
//TODO: create token
		{	if ( $raw_id.identifier != null && "raw".equals( $raw_id.identifier.getName() ) ) {
				...
				reportUnsupportedConstruct(
					new TITANMarker_V4("Invalid keyword, only the optional `raw' keyword can be used here.",
						$raw_id.start, $raw_id.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
			}
		}
		pr_RParen
	)?
...
//TODO: create token
				...
				reportUnsupportedConstruct(
					new TITANMarker_V4("Invalid indicator. Valid indicators are: `before', `value' and `after'",
						$id.start, $id.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
			}
		}
);
...
(	m = pr_TTCN3ModuleKeyword	{ col = $m.start; }
	i = pr_TTCN3ModuleId		{ act_ttcn3_module = new TTCN3Module_V4( $i.identifier, project ); }
	begin = pr_BeginChar
	...
	for ( int i = 0; i < $definitions.size(); i++ ) {
		...
		if ( modifier != null ) {
			...
			if( loc instanceof Location_V4 ) {
				((Location_V4)loc).setOffset($start);
			}
		}
		if( attributes != null ) {
			...
			Location loc = definition.getLocation();
			if( loc instanceof Location_V4 ) {
				((Location_V4)loc).setEndOffset(getStopToken());
			}
		}
	}
	if (commentLocation == null) {
...
@init {
	CompFieldMap compFieldMap = new CompFieldMap_V4();
}:
...
{
	reportUnsupportedConstruct(new TITANMarker_V4("Type parameterization is not currently supported",
		$col.start, $endcol.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
};
...
(	col = pr_MixedKeyword
		{
			...
			reportWarning(new TITANMarker_V4("Mixed ports are deprecated and may be fully removed in a future edition of the TTCN-3 standard ",
				$col.start, $col.stop, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
		}
	pr_BeginChar
	(	pr_MixedList[$body]
		pr_SemiColon?
...
)
{
	if ( $definitions.size() > 0 ) {
		...
		if ( pdef != null ) {
			...
			pdef.getLocation().setLine( t.getLine() );
...
{	for(int i = 0; i < $array.size(); i++) {
		...
		if(temp != null) {
			if (i==0) { // the location of "const Type" part belongs to the first const, no location overlapping
				temp.getLocation().setLine( $col.start.getLine() );
				Location loc = temp.getLocation();
				if( loc instanceof Location_V4 ) {
					((Location_V4)loc).setOffset( $col.start );
				}
			}
		}
	}
...
)
{
	if($i.identifier != null && altGuards != null ) {
		StatementBlock statementBlock = new StatementBlock_V4();
		statementBlock.setLocation(new LargeLocation_V4(actualFile, $blockstart.start, $endcol.stop));
		if(definitions != null) {
			for(Definition definition : definitions) {
...
pr_ExceptsDef:
	...
	endcol = pr_EndChar
{
	reportWarning(new TITANMarker_V4("Selective importation is not yet supported, importing all definitions",
										$col.start, $endcol.stop, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
};
...
{
	reportWarning(new TITANMarker_V4("Selective importation is not yet supported, importing all definitions",
		$start, getStopToken(), IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
};
...
pr_RecursiveKeyword:
	RECURSIVE
{
	reportWarning(new TITANMarker_V4("Recursive importation is deprecated and may be fully removed in a future edition of the TTCN-3 standard",
										$RECURSIVE, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
};

pr_ImportModuleId returns [ImportModule impmod = null]:
(	i = pr_ImportGlobalModuleId { $impmod = new ImportModule_V4($i.identifier); }
	pr_LanguageSpec?
);
...
(	PUBLIC?
	col = pr_GroupKeyword
	i = pr_Identifier
		{	group = new Group_V4($i.identifier);
	...
	if ( $parameters != null ) {
		for (int i = 0; i < $parameters.size(); i++) {
			...
			if ( def != null ) {
				...
				if( loc instanceof Location_V4 ) {
					((Location_V4)loc).setOffset($col.start);
					((Location_V4)loc).setEndOffset(endcol);
				}
			}
		}
	}
...
{
	if ( $block == null ) { $block = new StatementBlock_V4(); }
};
...
)
{
	if ( $definitions.size() > 0 ) {
		...
		if ( tdef != null ) {
			final Token t = $col.start;
			tdef.getLocation().setLine( t.getLine() );
...
(	a = pr_ToKeyword
	(	b = pr_AddressRef
			{
				ITTCN3Template template = $b.templateInstance.getTemplateBody();
				if ($b.templateInstance.getType() == null && $b.templateInstance.getDerivedReference() == null && template.getValue() != null) {
					$value = template.getValue();
				} else {
					reportUnsupportedConstruct(new TITANMarker_V4("Multicast communication is not currently supported",
						$b.start, $b.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
				}
			}
//	|	c = pr_AddressRefLis // covered by the previous rule
	|	d = pr_AllKeyword
		e = pr_ComponentKeyword
			{	reportUnsupportedConstruct(new TITANMarker_V4("Broadcast communication is not currently supported",
					$d.start, $e.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
			}
	)
);
...
|	CHARKEYWORD	{ $type = new CharString_Type();
										reportWarning(new TITANMarker_V4("Obsolete type `char' is taken as `charstring' ",
										$CHARKEYWORD, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
									}
|	UNIVERSAL
	(	CHARSTRING
	...
	|	CHARKEYWORD
			{ $type = new UniversalCharstring_Type();
				reportWarning(new TITANMarker_V4("Obsolete type `universal char' is taken as `universal charstring' ",
											$UNIVERSAL, $CHARKEYWORD, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
			}
	)
)
{
...
(	i = pr_Identifier { $identifier = $i.identifier; }
	(	t = pr_TypeActualParList
		{
			reportUnsupportedConstruct(new TITANMarker_V4("Reference to parameterized type is not currently supported ",
										$t.start, $t.stop, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
		}
	)?
);
...
//TODO: create token
		{	if (!"erroneous".equals($i.identifier.getName())) {
				...
				reportUnsupportedConstruct(
					new TITANMarker_V4("Invalid attribute. Valid attributes are: `encode', `variant' , `display' , `extension', `optional' and `erroneous'",
						$i.start, $i.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
			} else {
...
|	c = pr_AllRef
		{	reportUnsupportedConstruct(new TITANMarker_V4("Reference to multiple definitions in attribute qualifiers is not currently supported",
				$c.start, $c.stop, IMarker.SEVERITY_ERROR, IMarker.PRIORITY_NORMAL));
		}
);
...
{
	$statementblock.setLocation(new LargeLocation_V4(actualFile, $start, getStopToken()));
	if(statements != null) {
...
(	GOTO
	...
	|	ALT
			{
				reportWarning(
					new TITANMarker_V4("Obsolete statement `goto alt' will be substituted with `repeat' ",
						$ALT, IMarker.SEVERITY_WARNING, IMarker.PRIORITY_NORMAL));
			}
	)
)
{
...
|       s4 = pr_LoopConstruct			{ $statement = $s4.statement; }
|       s5 = pr_ConditionalConstruct	{ $statement = $s5.if_statement; }
|       s6 = pr_SelectCaseConstruct		{ $statement = $s6.statement; }
