...
// additional members and functions to the imported grammar (TTCN3Parser4.g4)
...
{	if ( impmod != null ) {
		...
		impmod.setLocation( new Location_V4( actualFile, $start, getStopToken() ) );
		if ( $parent_group == null ) {
...
(	PUBLIC?
	pr_GroupKeyword
	i = pr_Identifier	{ group = new Group_V4( $i.identifier ); }
	begin = pr_BeginChar
...
{	if ( group != null ) {
		...
		group.setInnerLocation( new Location_V4( actualFile, $begin.start, $end1.stop ) );
		if ( $parent_group != null ) {
...
{	for ( Identifier identifier : identifiers ) {
		FriendModule friend = new FriendModule_V4( identifier );
